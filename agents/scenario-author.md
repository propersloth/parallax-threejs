...
3a. If the finding being promoted is a confirmed memory leak fix (not a
   ...
   geometries *or* textures (checked independently, not summed — a
   geometry leak isn't maskable by unrelated texture disposal in the same
   run, or vice versa) vs. the last accepted baseline, per
   `test/visual/lib/types.ts`. `0` means zero tolerance for regrowth in
   either; only loosen it if the scenario's own interaction legitimately
