...
- **"I keep manually re-doing the same click-hover-drag just to check something."** `/replay` records the interaction once and reruns it exactly every time, so you're not left wondering if you hovered the same spot as last time — including touch/mobile drags and taps, not just mouse.
...
- **A real Chrome window pops open every session, unprompted.** `chrome-devtools-mcp` and `playwright-mcp` default to headed (visible) browsers upstream; versions of `.mcp.json` scaffolded before this was fixed don't pass `--headless`. `/init` never overwrites an existing `.mcp.json`, so re-running it won't pick up the fix — add `"--headless"` to both servers' `args` in your project's own `.mcp.json` by hand. (Skip this if you're on the Raspberry Pi example config — that one runs headed on purpose; see `AGENTS.md` §7a.)
