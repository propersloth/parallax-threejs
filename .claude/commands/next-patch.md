---
description: Cut a next (pre-release) build with a patch version bump, via release.yml
allowed-tools: Bash(gh workflow run:*), Bash(gh run list:*), Bash(gh run watch:*)
---

Trigger a `next` release with a patch bump:

    gh workflow run release.yml -f mode=next -f bump_type=patch
