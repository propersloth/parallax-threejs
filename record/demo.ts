...
// runs the REAL /checkpoint and /diff commands (templates/scripts/
...
// request.
//
// Usage: deno run --allow-all record/demo.ts <scenario-name> [--pipeline <name>]

import { chromium } from "playwright";
import { runSteps } from "../templates/test/visual/lib/capture.ts";
import type { Scenario, Step } from "../templates/test/visual/lib/types.ts";
...
if (!scenarioName || scenarioName.startsWith("--")) {
  console.error(
    "usage: record/demo.ts <scenario-name> [--pipeline <name>]",
  );
  Deno.exit(1);
}
...
const rawDir = ".record-output/raw";
const outDir = ".record-output/videos";
...
// within templates/ -- matching how these scripts expect to run inside
...
> {
  const CDP_PORT = "9223"; // matches templates/scripts/lib/live-scene.ts's BRIDGE_PORT default
  ...
  try {
    ...
    const diffsDir = "templates/.parallax/diffs";
    ...
    // "templates/" prefix, which only reflects where *this* throwaway
    ...
    await Deno.remove("templates/.parallax/checkpoints", { recursive: true })
      .catch(() => {});
    await Deno.remove("templates/.parallax/diffs", { recursive: true }).catch(
      () => {},
    );
    await Deno.remove("templates/.parallax/reports", { recursive: true })
      .catch(() => {});
